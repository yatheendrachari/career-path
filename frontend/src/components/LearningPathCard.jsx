...
//       {showTimeline && (
//         <div className="absolute left-6 top-0 bottom-0 w-0.5 bg-blue-700 -z-10" />
//       )}
...
//         {/* Card Content */}
//         <div
//           className={`bg-linear-to-br ${gradient} border-2 ${borderColor} rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 ${
...
const LearningPathCard = ({
  ...
  return (
    <div className="relative">
      {/* Timeline Connector (optional) */}
      {showTimeline && (
        <div className="absolute left-6 top-0 bottom-0 w-1 bg-indigo-600 -z-10 shadow-md" />
      )}
      ...
      <div className="relative pl-14 pb-8">
        {/* Timeline Dot */}
        {showTimeline && (
          <div className={`absolute left-0 top-2 w-12 h-12 ${iconColor} rounded-full flex items-center justify-center text-white font-bold shadow-xl border-3 border-white`}>
            {index + 1}
          </div>
        )}
        ...
        {/* Card Content */}
        <div
          className={`bg-linear-to-br ${gradient} border-3 ${borderColor} rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300 ${
        ...
        >
          ...
          <div className="p-6">
            <div className="flex items-start justify-between mb-3">
              ...
              {hasResources && (
                <button
                  onClick={() => setIsExpanded(!isExpanded)}
                  className="ml-4 p-2 hover:bg-white/60 rounded-xl transition-colors border-2 border-gray-900"
                  aria-label={isExpanded ? 'Collapse details' : 'Expand details'}
                >
                  <svg
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-2">
                  {topics.map((topic, idx) => (
                    ...
                    >
                      <svg className="w-6 h-6 text-emerald-700 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                      </svg>
                      <span className="text-base leading-relaxed font-semibold">{topic}</span>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          {hasResources && (
            ...
            >
              <div className="border-t-3 border-gray-500 bg-white/40 p-6">
                <div className="flex items-center gap-2 mb-3">
                ...
                <ul className="space-y-2">
                  {resources.map((resource, idx) => (
                    <li
                      ...
                      className="flex items-start gap-2 text-gray-900"
                    >
                      <svg className="w-5 h-5 text-indigo-700 flex-shrink-0 mt-1" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clipRule="evenodd" />
                      </svg>
          ...
          {/* Expand Hint (when collapsed and has resources) */}
          {!isExpanded && hasResources && (
            <div className="border-t-3 border-gray-500 bg-white/30 px-6 py-3">
              <button
                onClick={() => setIsExpanded(true)}
                className="text-sm font-bold text-gray-900 hover:text-indigo-700 transition-colors flex items-center gap-1"
              >
                <span>View Resources</span>
